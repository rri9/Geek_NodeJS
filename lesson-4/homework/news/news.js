...
const categories = ['all', 'world', 'russia', 'sport'];

hbs.registerHelper('isSelected', (category, key) => (category === key ? 'selected' : ''));
...
app.get('/news', (req, res) => {
  ...
  if (isObjEmpty(req.query)) {
    //Приоритет передаваемым параметрам get-запроса
...
app.post('/news', (req, res) => {
  ...
  getNews(url)
    .then(newsObj => getFilteredNews(newsObj, req.body))
    .then(filteredNewsObj => {
      res.cookie('category', filteredNewsObj.cookies.category, { maxAge: 1000 * 60 * 60 * 24 * 1 });
      res.cookie('newsAmount', filteredNewsObj.cookies.newsAmount, {
